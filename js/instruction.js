...
var roundPoints,      // кол-во указателей действий на текущем изобращении
    ...
    currentParagraph = 0,  // индекс текущего параграфа
...
// переключение на следующий или предыдущий параграф
$('.theEndButtons')[0].addEventListener('click', function () {
  $("#menu .chapter li")[currentParagraph-1].dispatchEvent(eventClick);
});

$('.theEndButtons')[2].addEventListener('click', function () {
  window.close();
  window.opener.close();
});

$('.theEndButtons')[1].addEventListener('click', function () {
  $("#menu .chapter li")[currentParagraph+1].dispatchEvent(eventClick);
});
